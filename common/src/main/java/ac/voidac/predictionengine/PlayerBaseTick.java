...
import ac.voidac.utils.data.attribute.ValuedAttribute;
...
import ac.voidac.utils.latency.CompensatedEntities;
...
import ac.voidac.utils.nmsutil.CheckIfChunksLoaded;
...
public final class PlayerBaseTick {
    ...
    public static boolean canEnterPose(VoidPlayer player, Pose pose, double x, double y, double z) {
        return Collisions.isEmpty(player, getBoundingBoxForPose(player, pose, x, y, z).expand(-1.0E-7D));
    }

    private static SimpleCollisionBox getBoundingBoxForPose(VoidPlayer player, Pose pose, double x, double y, double z) {
    ...
    public static void doBaseTick(VoidPlayer player) {
    ...
    private static void updateFluidOnEyes(VoidPlayer player) {
    ...
    private static void updateInWaterStateAndDoFluidPushing(VoidPlayer player) {
    ...
    public static void updatePowderSnow(VoidPlayer player) {
    ...
    // 1.14
    public static void updatePlayerPose(VoidPlayer player) {
    ...
    // 1.13 and below
    private static void updatePlayerSize(VoidPlayer player) {
        Pose pose;
        if (player.isGliding) {
            pose = Pose.FALL_FLYING;
        } else if (player.isInBed) {
    ...
    private static void updateSwimming(VoidPlayer player) {
    ...
    private static void moveTowardsClosestSpace(VoidPlayer player, double xPosition, double zPosition) {
    ...
    private static void moveTowardsClosestSpaceLegacy(VoidPlayer player, double x, double z) {
        int floorX = VoidMath.floor(x);
        int floorZ = VoidMath.floor(z);
        int floorY = VoidMath.floor(player.lastY + 0.5);
    ...
    // 1.14+
    private static void moveTowardsClosestSpaceModern(VoidPlayer player, double xPosition, double zPosition) {
    ...
    public static void updateInWaterStateAndDoWaterCurrentPushing(VoidPlayer player) {
        final PacketEntity riding = player.compensatedEntities.self.getRiding();
    ...
    private static boolean updateFluidHeightAndDoFluidPushing(VoidPlayer player, FluidTag tag, double multiplier) {
    ...
    private static boolean updateFluidHeightAndDoFluidPushingLegacy(VoidPlayer player, FluidTag tag, double multiplier) {
        SimpleCollisionBox aABB = player.boundingBox.copy().expand(0, -0.4, 0).expand(-0.001);

        int floorX = VoidMath.floor(aABB.minX);
        int ceilX = VoidMath.ceil(aABB.maxX);
        int floorY = VoidMath.floor(aABB.minY);
        int ceilY = VoidMath.ceil(aABB.maxY);
        int floorZ = VoidMath.floor(aABB.minZ);
        int ceilZ = VoidMath.ceil(aABB.maxZ);
    ...
    private static boolean updateFluidHeightAndDoFluidPushingModern(VoidPlayer player, FluidTag tag, double multiplier) {
        ...
        for (int x = floorX; x < ceilX; ++x) {
            for (int y = floorY; y < ceilY; ++y) {
                for (int z = floorZ; z < ceilZ; ++z) {
                    ...
                    // Vanilla keeps fluid heights in float; widening to double here made
                    // the clamp below round differently than the client did.
                    float fluidHeight;
                    if (tag == FluidTag.WATER) {
                        fluidHeight = player.compensatedWorld.getWaterFluidLevelAt(x, y, z);
                    } else {
    ...
    public static boolean updateFluidInteraction(VoidPlayer player) {
    ...
    private static boolean suffocatesAt(VoidPlayer player, int x, int z) {
    ...
    private static boolean clearAbove(VoidPlayer player, int x, int y, int z) {
