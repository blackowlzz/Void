...
import ac.voidac.checks.impl.vehicle.VehicleC;
import ac.voidac.checks.type.PositionCheck;
import ac.voidac.manager.SetbackTeleportUtil;
import ac.voidac.player.VoidPlayer;
import ac.voidac.predictionengine.movementtick.MovementTickerCamel;
import ac.voidac.predictionengine.movementtick.MovementTickerHappyGhast;
import ac.voidac.predictionengine.movementtick.MovementTickerHorse;
import ac.voidac.predictionengine.movementtick.MovementTickerNautilus;
import ac.voidac.predictionengine.movementtick.MovementTickerPig;
import ac.voidac.predictionengine.movementtick.MovementTickerPlayer;
import ac.voidac.predictionengine.movementtick.MovementTickerStrider;
import ac.voidac.predictionengine.predictions.PredictionEngineNormal;
import ac.voidac.predictionengine.predictions.rideable.PredictionEngineBoat;
import ac.voidac.predictionengine.predictions.rideable.PredictionEngineRideableUtils;
...
import ac.voidac.utils.data.packetentity.PacketEntityRideable;
import ac.voidac.utils.data.packetentity.PacketEntityTrackXRot;
...
public class MovementCheckRunner extends Check implements PositionCheck {
    ...
    public MovementCheckRunner(VoidPlayer player) {
        super(player);
    }
    ...
    private void handleTeleport(PositionUpdate update) {
        ...
        //
        // However, for some reason, this behaviour does not work correctly in Void
    ...
    private void check(PositionUpdate update) {
        ...
        if (!player.isFlying && player.isSneaking && Collisions.isAboveGround(player)) {
            ...
            double posX = Math.max(0.05, VoidMath.clamp(player.actualMovement.getX(), -16, 16) + 0.05);
            double posZ = Math.max(0.05, VoidMath.clamp(player.actualMovement.getZ(), -16, 16) + 0.05);
            double negX = Math.min(-0.05, VoidMath.clamp(player.actualMovement.getX(), -16, 16) - 0.05);
            double negZ = Math.min(-0.05, VoidMath.clamp(player.actualMovement.getZ(), -16, 16) - 0.05);
        ...
        if (player.vehicleData.wasVehicleSwitch || player.vehicleData.lastDummy) {
            ...
            if (riding != null) {
                ...
                // their similar code may also work on void.
                //
...
//                    player.uncertaintyHandler.isSteppingOnSlime = true;
        ...
        } else if (player.disableVoid || (PacketEvents.getAPI().getServerManager().getVersion().isNewerThanOrEquals(ServerVersion.V_1_8) && player.gamemode == GameMode.SPECTATOR) || player.isFlying || (player.isExemptElytra() && player.isGliding)) {
