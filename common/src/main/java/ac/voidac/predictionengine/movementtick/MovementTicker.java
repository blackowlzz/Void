...
import ac.voidac.predictionengine.PlayerBaseTick;
import ac.voidac.predictionengine.predictions.PredictionEngine;
import ac.voidac.predictionengine.predictions.PredictionEngineElytra;
...
import ac.voidac.utils.nmsutil.EntityTypeTags;
import ac.voidac.utils.nmsutil.FluidFallingAdjustedMovement;
import ac.voidac.utils.nmsutil.GetBoundingBox;
import ac.voidac.utils.nmsutil.MainSupportingBlockPosFinder;
import ac.voidac.utils.viaversion.ViaVersionUtil;
import ac.voidac.utils.team.EntityPredicates;
import ac.voidac.utils.team.EntityTeam;
...
public class MovementTicker {
    public final VoidPlayer player;

    public static void handleEntityCollisions(VoidPlayer player) {
    ...
    private boolean isHorizontalCollisionSoft(Vector3dm collide) {
        ...
        if (input != null) {
            return isHorizontalCollisionSoft(collide, horizontalLengthSquared, sin, cos,
                    (float) input.getX(), (float) input.getZ());
        }

        // gliding, so we never got a single input to work from. try the lot and
        // take a soft hit if any of them would have been one
        List<VectorData> results = new ArrayList<>();
        new PredictionEngine().loopVectors(player,
                Set.of(new VectorData(new Vector3dm(), VectorData.VectorType.Normal)),
                0, results, false);

        for (VectorData data : results) {
            if (isHorizontalCollisionSoft(collide, horizontalLengthSquared, sin, cos,
    ...
    private boolean isHorizontalCollisionSoft(Vector3dm collide, double horizontalLengthSquared,
                                              double sin, double cos, float xxa, float zza) {
    ...
    public void move(Vector3dm inputVel, Vector3dm collide) {
        ...
        if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_18_2)) {
            boolean xAxis = !VoidMath.equal(inputVel.getX(), collide.getX());
        ...
        if (collide.lengthSquared() <= 1e-7
            ...
            player.addMovementThisTick(new VoidPlayer.Movement(from, to, new Vector3d(inputVel.getX(), inputVel.getY(), inputVel.getZ())));
        }
    ...
    public void livingEntityTravel() {
        ...
        double lavaLevel = 0;
        if (canStandOnLava())
            lavaLevel = player.compensatedWorld.getLavaFluidLevelAt(VoidMath.floor(player.lastX), VoidMath.floor(player.lastY), VoidMath.floor(player.lastZ));
        ...
        if (player.wasTouchingWater && !player.isFlying) {
            ...
            player.friction = swimFriction; // Not vanilla, just useful for other void stuff
            ...
            if (player.wasTouchingLava && !player.isFlying && !(lavaLevel > 0 && canStandOnLava())) {
                player.friction = 0.5F; // Not vanilla, just useful for other void stuff
                ...
                if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_5) && Collisions.onClimbable(player, player.lastX, player.lastY, player.lastZ)) {
                    ...
                    player.friction = 0.99F; // Not vanilla, just useful for other void stuff
