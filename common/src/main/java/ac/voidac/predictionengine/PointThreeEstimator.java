 ...
 * Someone trying to false void by using negative levitation effects
...
public class PointThreeEstimator {
    ...
    public PointThreeEstimator(VoidPlayer player) {
        this.player = player;
    }
    ...
    public void handleChangeBlock(int x, int y, int z, WrappedBlockState state) {
        ...
        if (pointThreeBox.isIntersected(new SimpleCollisionBox(x, y, z))) {
            // https://github.com/MWHunter/Void/issues/613
    ...
    private double iterateGravity(VoidPlayer player, double y) {
