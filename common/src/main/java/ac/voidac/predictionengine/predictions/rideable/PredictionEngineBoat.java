...
import ac.voidac.utils.enums.BoatEntityStatus;
...
public class PredictionEngineBoat extends PredictionEngine {
    public PredictionEngineBoat(VoidPlayer player) {
    ...
    private static BoatEntityStatus getStatus(VoidPlayer player) {
    ...
    private static @Nullable BoatEntityStatus isUnderwater(@NotNull VoidPlayer player) {
        SimpleCollisionBox box = player.boundingBox;
        double maxBoxY = box.maxY + 0.001D;
        int minX = VoidMath.floor(box.minX);
        int maxX = VoidMath.ceil(box.maxX);
        int minY = VoidMath.floor(box.maxY);
        int maxY = VoidMath.ceil(maxBoxY);
        int minZ = VoidMath.floor(box.minZ);
        int maxZ = VoidMath.ceil(box.maxZ);
    ...
    private static boolean checkInWater(VoidPlayer voidPlayer) {
        SimpleCollisionBox box = voidPlayer.boundingBox;
        int minX = VoidMath.floor(box.minX);
        int maxX = VoidMath.ceil(box.maxX);
        int minY = VoidMath.floor(box.minY);
        int maxY = VoidMath.ceil(box.minY + 0.001D);
        int minZ = VoidMath.floor(box.minZ);
        int maxZ = VoidMath.ceil(box.maxZ);
        boolean inWater = false;
        voidPlayer.vehicleData.waterLevel = -Double.MAX_VALUE;

        for (int x = minX; x < maxX; ++x) {
            for (int y = minY; y < maxY; ++y) {
                for (int z = minZ; z < maxZ; ++z) {
                    double level = voidPlayer.compensatedWorld.getWaterFluidLevelAt(x, y, z);
                    if (level > 0) {
                        float f = (float) ((float) y + level);
                        voidPlayer.vehicleData.waterLevel = Math.max(f, voidPlayer.vehicleData.waterLevel);
    ...
    public static float getGroundFriction(VoidPlayer player) {
    ...
    private void floatBoat(VoidPlayer player, Vector3dm vector) {
    ...
    public float getWaterLevelAbove(VoidPlayer player) {
    ...
    private void controlBoat(VoidPlayer player, Vector3dm vector, boolean intermediate) {
        float f = 0.0F;
        ...
        vector.add(new Vector3dm(player.trigHandler.sin(VoidMath.radians(-player.yaw)) * f, 0, (double) (player.trigHandler.cos(VoidMath.radians(player.yaw)) * f)));
    }
}
