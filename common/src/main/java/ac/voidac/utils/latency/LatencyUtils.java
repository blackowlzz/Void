...
public class LatencyUtils {
    ...
    public LatencyUtils(VoidPlayer player) {
        this.player = player;
    }
        ...
        synchronized (this) {
            ...
            for (Runnable runnable : tasksToRun) {
                try {
                    ...
                    if (CommonVoidArguments.KICK_ON_TRANSACTION_ERRORS.value()) {
                        player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, VoidAPI.INSTANCE.getConfigManager().getDisconnectPacketError())));
                    }
                }
            }
        }
