package ac.voidac.utils.collisions.blocks.connecting;
...
public class DynamicHitboxWall extends DynamicConnecting implements HitBoxFactory {
    @Override
    public CollisionBox fetch(VoidPlayer player, StateType heldItem, ClientVersion version, WrappedBlockState state, boolean isTargetBlock, int x, int y, int z) {
    ...
    private int[] getConnections(VoidPlayer player, ClientVersion version, WrappedBlockState state, int x, int y, int z) {
    ...
    @Override
    public boolean checkCanConnect(VoidPlayer player, WrappedBlockState state, StateType one, StateType two, BlockFace direction) {
