...
import ac.voidac.utils.nmsutil.Ray;
...
public class SimpleCollisionBox implements CollisionBox {
     ...
     */
    // Ray-AABB intersection via slab method (adapted from Hawk anticheat)
    ...
    public Vector3d getCenter() {
        return new Vector3d(VoidMath.lerp(0.5, this.minX, this.maxX), VoidMath.lerp(0.5, this.minY, this.maxY), VoidMath.lerp(0.5, this.minZ, this.maxZ));
    }
    ...
    public static Vector3i containing(double x, double y, double z) {
        return new Vector3i(VoidMath.floor(x), VoidMath.floor(y), VoidMath.floor(z));
    }
    ...
    public static Iterable<Vector3i> betweenCornersInDirection(SimpleCollisionBox boundingBox, Vector3d directionVector) {
        ...
        int minX = VoidMath.floor(min.x);
        int minY = VoidMath.floor(min.y);
        int minZ = VoidMath.floor(min.z);
        Vector3d max = boundingBox.max().toVector3d();
        int maxX = VoidMath.floor(max.x);
        int maxY = VoidMath.floor(max.y);
        int maxZ = VoidMath.floor(max.z);
