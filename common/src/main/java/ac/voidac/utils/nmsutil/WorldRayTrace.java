...
public class WorldRayTrace {
    public static HitData getNearestBlockHitResult(VoidPlayer player, StateType heldItem, boolean sourcesHaveHitbox, boolean fluidPlacement, boolean itemUsePlacement) {
    ...
    public static HitData traverseBlocks(VoidPlayer player, Vector3d start, Vector3d end, BiFunction<WrappedBlockState, Vector3i, HitData> predicate) {
        // I guess go back by the collision epsilon?
        double endX = VoidMath.lerp(-1.0E-7D, end.x, start.x);
        double endY = VoidMath.lerp(-1.0E-7D, end.y, start.y);
        double endZ = VoidMath.lerp(-1.0E-7D, end.z, start.z);
        double startX = VoidMath.lerp(-1.0E-7D, start.x, end.x);
        double startY = VoidMath.lerp(-1.0E-7D, start.y, end.y);
        double startZ = VoidMath.lerp(-1.0E-7D, start.z, end.z);
        int floorStartX = VoidMath.floor(startX);
        int floorStartY = VoidMath.floor(startY);
        int floorStartZ = VoidMath.floor(startZ);
        ...
        double d12 = posXInverse * (xSign > 0 ? 1.0D - VoidMath.frac(startX) : VoidMath.frac(startX));
        double d13 = posYInverse * (ySign > 0 ? 1.0D - VoidMath.frac(startY) : VoidMath.frac(startY));
        double d14 = posZInverse * (zSign > 0 ? 1.0D - VoidMath.frac(startZ) : VoidMath.frac(startZ));
