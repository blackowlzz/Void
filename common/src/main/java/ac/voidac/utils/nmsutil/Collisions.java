...
import ac.voidac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_10;
import ac.voidac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_2;
import ac.voidac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_4;
import ac.voidac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_5;
import ac.voidac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_6;
...
public final class Collisions {
    ...
    public static boolean slowCouldPointThreeHitGround(VoidPlayer player, double x, double y, double z) {
    ...
    public static Vector3dm collide(VoidPlayer player, double desiredX, double desiredY, double desiredZ) {
        return collide(player, desiredX, desiredY, desiredZ, desiredY, null);
    }

    public static Vector3dm collide(VoidPlayer player, double desiredX, double desiredY, double desiredZ, double clientVelY, VectorData data) {
    ...
    public static boolean addWorldBorder(VoidPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    public static boolean getCollisionBoxes(VoidPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    public static boolean isEmpty(VoidPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static Vector3dm maybeBackOffFromEdge(Vector3dm vec3, VoidPlayer player, boolean overrideVersion) {
    ...
    public static boolean isAboveGround(VoidPlayer player) {
    ...
    public static void handleInsideBlocks(VoidPlayer player) {
    ...
    public static void applyEffectsFromBlocks(VoidPlayer player) {
        ...
        if (clientVersion.isOlderThan(ClientVersion.V_1_21_5)) {
            player.finalMovementsThisTick.add(new VoidPlayer.Movement(from, to));
            ...
            if (player.finalMovementsThisTick.isEmpty()) {
                ...
                player.finalMovementsThisTick.add(new VoidPlayer.Movement(player.finalMovementsThisTick.get(player.finalMovementsThisTick.size() - 1).to(), to));
            }
        }
    ...
    public static void resolveBlockEffects(VoidPlayer player, Vector3d from, Vector3d to) {
        Collisions.resolveBlockEffects(player, List.of(new VoidPlayer.Movement(from, to)));
    }

    public static void resolveBlockEffects(VoidPlayer player, List<VoidPlayer.Movement> movements) {
    ...
    private static double getOldDeltaY(VoidPlayer player, double value) {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_2) ? value / 0.98F + 0.08 : value;
    }

    private static double getNewDeltaY(VoidPlayer player, double value) {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_2) ? (value - 0.08) * 0.98F : value;
    }

    private static boolean isSlidingDown(Vector3dm vector, VoidPlayer player, int locationX, int locationY,
                                         int locationZ) {
        if (player.onGround) {
            return false;
    ...
    // 0.03 hack
    public static boolean checkStuckSpeed(VoidPlayer player, double expand) {
        // Use the bounding box for after the player's movement is applied
    ...
    public static boolean suffocatesAt(VoidPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static boolean doesBlockSuffocate(VoidPlayer player, int x, int y, int z) {
        WrappedBlockState data = player.compensatedWorld.getBlock(x, y, z);
    ...
    // Thanks Tuinity
    public static boolean hasMaterial(VoidPlayer player, SimpleCollisionBox checkBox, Predicate<Pair<WrappedBlockState, Vector3i>> searchingFor) {
    ...
    // Thanks Tuinity
    public static void forEachCollisionBox(@NotNull VoidPlayer player, @NotNull SimpleCollisionBox checkBox, @NotNull BiConsumer<WrappedBlockState, Vector3i> searchingFor) {
    ...
    public static boolean onClimbable(VoidPlayer player, double x, double y, double z) {
    ...
    public static boolean trapdoorUsableAsLadder(VoidPlayer player, double x, double y, double z, WrappedBlockState blockData) {
