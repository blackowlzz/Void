...
import ac.voidac.utils.data.tags.SyncedTag;
import ac.voidac.utils.data.tags.SyncedTags;
import ac.voidac.utils.enums.FluidTag;
import ac.voidac.utils.inventory.EnchantmentHelper;
...
public class BlockBreakSpeed {
    ...
    public static double getBlockDamage(VoidPlayer player, WrappedBlockState block) {
        ItemStack tool = player.inventory.getHeldItem();
        return getBlockDamage(player, tool, block.getType());
    }

    public static double getBlockDamage(VoidPlayer player, ItemStack tool, StateType block) {
        // GET destroy speed
    ...
    private static float getSpeedMultiplierFromToolData(VoidPlayer player, ItemStack tool, ToolSpeedData data) {
    ...
    private static ToolSpeedData getModernToolSpeedData(VoidPlayer player, ItemStack tool, StateType block) {
    ...
    private static ToolSpeedData getLegacyToolSpeedData(VoidPlayer player, ItemStack tool, StateType block) {
        ItemType toolType = tool.getType();
        float speedMultiplier = 1.0f;
        boolean isCorrectToolForDrop = false;
