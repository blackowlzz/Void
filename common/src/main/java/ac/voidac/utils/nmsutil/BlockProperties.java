...
import ac.voidac.utils.data.packetentity.PacketEntityStrider;
...
public class BlockProperties {
     ...
     */
    public static StateType getOnPos(VoidPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static float getFriction(VoidPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)) {
    ...
    public static float getBlockSpeedFactor(VoidPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static boolean onHoneyBlock(VoidPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
     ...
     */
    private static StateType getBlockPosBelowThatAffectsMyMovement(VoidPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        Vector3i pos = player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)
                ? new Vector3i(VoidMath.floor(playerPos.getX()), VoidMath.floor(playerPos.getY() - 0.5000001), VoidMath.floor(playerPos.getZ()))
                : getOnPos(player, playerPos, mainSupportingBlockData, 0.500001F);
        return player.compensatedWorld.getBlockType(pos.x, pos.y, pos.z);
    }

    private static Vector3i getOnPos(VoidPlayer player, Vector3d playerPos, MainSupportingBlockData mainSupportingBlockData, float searchBelowPlayer) {
        ...
        if (mainBlockPos != null) {
            ...
            return shouldReturn ? mainBlockPos.withY(VoidMath.floor(playerPos.getY() - (double) searchBelowPlayer)) : mainBlockPos;
        } else {
            return new Vector3i(VoidMath.floor(playerPos.getX()), VoidMath.floor(playerPos.getY() - searchBelowPlayer), VoidMath.floor(playerPos.getZ()));
        }
    }

    public static float getMaterialFriction(VoidPlayer player, StateType material) {
        float friction = 0.6f;
    ...
    private static StateType getOnBlock(VoidPlayer player, double x, double y, double z) {
        StateType block1 = player.compensatedWorld.getBlockType(VoidMath.floor(x), VoidMath.floor(y - 0.2F), VoidMath.floor(z));

        if (block1.isAir()) {
            StateType block2 = player.compensatedWorld.getBlockType(VoidMath.floor(x), VoidMath.floor(y - 1.2F), VoidMath.floor(z));
    ...
    private static float getBlockSpeedFactorLegacy(VoidPlayer player, Vector3d pos) {
    ...
    private static float getBlockSpeedFactor(VoidPlayer player, StateType type) {
