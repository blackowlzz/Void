...
import ac.voidac.utils.blockstate.helper.BlockFaceHelper;
...
import ac.voidac.utils.nmsutil.Dripstone;
...
public enum BlockPlaceResult {
    ...
    TORCH_OR_HEAD((player, place) -> {
        // type doesn't matter to void, same hitbox.
    ...
    // As long as one of these properties is met, it is good enough for void!
    VINE((player, place) -> {
    ...
    DOOR((player, place) -> {
        if (place.isFullFace(BlockFace.DOWN) && place.isBlockFaceOpen(BlockFace.UP)) {
            ...
            if (PacketEvents.getAPI().getServerManager().getVersion().isNewerThanOrEquals(ServerVersion.V_1_13)) { // Only works on 1.13+
                ...
                // Use strings: PacketEvents doesn't support one material with two sets of data
    ...
    SCAFFOLDING((player, place) -> {
        ...
        } // else, cancel if the scaffolding is exactly 7 away, void doesn't handle this edge case yet.
