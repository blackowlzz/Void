...
import ac.voidac.checks.impl.sprint.SprintD;
...
public class PacketEntitySelf extends PacketEntity {
    ...
    public PacketEntitySelf(VoidPlayer player) {
        super(player, EntityTypes.PLAYER);
        this.player = player;
    }

    public PacketEntitySelf(VoidPlayer player, PacketEntitySelf old) {
        super(player, EntityTypes.PLAYER);
        this.player = player;
    ...
    protected void initAttributes(VoidPlayer player) {
        ...
        trackAttribute(ValuedAttribute.ranged(Attributes.SNEAKING_SPEED, 0.3, 0, 1)
                .withGetRewriter(value -> {
                    ...
                    final double clamped = VoidMath.clamp(0.3f + swiftSneak * 0.15f, 0f, 1f);
                    if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) {
                        return clamped;
                    }
