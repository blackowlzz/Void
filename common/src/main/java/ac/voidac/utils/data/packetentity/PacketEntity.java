...
import ac.voidac.utils.data.ReachInterpolationData;
import ac.voidac.utils.data.TrackedPosition;
...
public class PacketEntity extends TypedPacketEntity {
    ...
    public PacketEntity(VoidPlayer player, EntityType type) {
        super(type);
        this.uuid = null;
    ...
    public PacketEntity(VoidPlayer player, UUID uuid, EntityType type, double x, double y, double z) {
        super(type);
        this.uuid = uuid;
        initAttributes(player);
        this.trackedServerPosition = new TrackedPosition();
    ...
    protected void initAttributes(VoidPlayer player) {
        ...
        trackAttribute(ValuedAttribute.ranged(Attributes.GRAVITY, 0.08, -1, 1)
                .requiredVersion(player, ClientVersion.V_1_20_5));
        // 26.2 physics attributes. They are tracked on every entity regardless of version so
        // that reading them never throws; requiredVersion pins them to these defaults below
        // 26.2, and the defaults make the formulas that consume them the identity.
    ...
    public void setPositionRaw(VoidPlayer player, SimpleCollisionBox box) {
