package ac.voidac.utils.data.attribute;
...
import static ac.voidac.utils.latency.CompensatedEntities.SPRINTING_MODIFIER_UUID;
...
public final class ValuedAttribute {
     ...
     */
    public ValuedAttribute requiredVersion(VoidPlayer player, ClientVersion requiredVersion) {
    ...
    public double with(WrapperPlayServerUpdateAttributes.Property property) {
        ...
        double newValue =
                VoidMath.clamp((baseValue + additionSum) * (1 + multiplyBaseSum) * multiplyTotalProduct,
                        min, max);
        if (setRewriter != null) {
