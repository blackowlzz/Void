package ac.voidac.utils.data.webhook.discord;
...
public record CompiledDiscordTemplate(Segment[] segments) {
    ...
    /**
     * @param player              Void player (nullable)
     ...
     */
    public String render(@NotNull VoidPlayer player,
                         @NotNull Map<String, String> statics,
                         @NotNull Map<String, Function<VoidUser, String>> dynamics,
        ...
        for (Segment seg : segments) {
            if (seg instanceof Literal l) {
                ...
                if (val == null) {
                    Function<VoidUser, String> fn = dynamics.get(p.key);
                    if (fn != null) val = fn.apply(player);
                }

                if (val == null) {
                    String resolved = VoidAPI.INSTANCE.getMessagePlaceHolderManager()
