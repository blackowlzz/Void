package ac.voidac.utils.anticheat.update;

import ac.voidac.player.VoidPlayer;
import ac.voidac.utils.anticheat.LogUtil;
import ac.voidac.utils.collisions.AxisSelect;
...
public class BlockPlace {
    ...
    public BlockPlace(VoidPlayer player, InteractionHand hand, Vector3i position, int faceId, BlockFace face, ItemStack itemStack, @Nullable HitData hitData, int sequence) {
        this.player = player;
        this.hand = hand;
    ...
    private List<BlockFace> getNearestLookingDirections() {
        float pitch = VoidMath.radians(player.pitch);
        float yaw = VoidMath.radians(-player.yaw);
    ...
    public BlockFace getPlayerFacing() {
        return BY_2D[VoidMath.floor(player.yaw / 90.0D + 0.5D) & 3];
    }
