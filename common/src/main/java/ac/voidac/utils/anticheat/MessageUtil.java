...
public class MessageUtil {
    ...
    public @Nullable String replacePlaceholders(@Nullable VoidPlayer player, @Nullable String string, boolean removeFormatting) {
        return replacePlaceholders(player, player == null ? null : player.platformPlayer, string, removeFormatting);
    }

    @Contract("_, null -> null; _, !null -> !null")
    public @Nullable String replacePlaceholders(@Nullable VoidPlayer player, @Nullable String string) {
    ...
    public @Nullable String replacePlaceholders(@Nullable PlatformPlayer player, @Nullable String string) {
        return replacePlaceholders(player == null ? null : VoidAPI.INSTANCE.getPlayerDataManager().getPlayer(player.getUniqueId()), player, string, false);
    }
    ...
    private @Nullable String replacePlaceholders(@Nullable VoidPlayer voidPlayer, @Nullable PlatformPlayer platformPlayer, @Nullable String string, boolean removeFormatting) {
        ...
        if (!matcher.find()) {
            return VoidAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, string);
        }

        // Get references to the maps once, outside the loop.
        final Map<String, String> staticReplacements = VoidAPI.INSTANCE.getExternalAPI().getStaticReplacements();
        final Map<String, Function<VoidUser, String>> variableReplacements = VoidAPI.INSTANCE.getExternalAPI().getVariableReplacements();
        ...
        String voidReplaced = sb.toString();

        return VoidAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, voidReplaced).replace(PLACEHOLDER_ESCAPE_CHAR, '%');
    }

    public @NotNull Component replacePlaceholders(@NotNull VoidPlayer player, @NotNull Component component) {
    ...
    public @NotNull Component miniMessage(@NotNull String string) {
        string = string.replace("%prefix%", VoidAPI.INSTANCE.getConfigManager().getConfig().getStringElse("prefix", "&8[&5Void&8]"));
    ...
    public Component getParsedComponent(Sender sender, String key, String fallbackText) {
        String message = VoidAPI.INSTANCE.getConfigManager().getConfig().getStringElse(key, fallbackText);
