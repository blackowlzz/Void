 ...
 * Void uses a hybrid system for inventories - we lag compensate but rely on the server
...
public class CorrectingPlayerInventoryStorage extends InventoryStorage {
    ...
    public CorrectingPlayerInventoryStorage(VoidPlayer player, int size) {
        super(size);
        this.player = player;
    }
    ...
    public void handleClientClaimedSlotSet(int slotID) {
        if (slotID >= 0 && slotID <= Inventory.ITEMS_END) {
            pendingFinalizedSlot.put(slotID, VoidAPI.INSTANCE.getTickManager().currentTick + 5);
        }
    }
    ...
    // Required for pre-1.17 clients and also 1.17 due to inventory netcode changes
    @Override
    public void setItem(int item, ItemStack stack) {
        ...
        if (finalTransaction == -1 || player.lastTransactionReceived.get() >= finalTransaction) {
            ...
            pendingFinalizedSlot.put(item, VoidAPI.INSTANCE.getTickManager().currentTick + 5);
    ...
    public void tickWithBukkit() {
        ...
        int tickID = VoidAPI.INSTANCE.getTickManager().currentTick;
        ...
        // If the player's inventory needs to be resent so that Void can enable the player's packet inventory again
