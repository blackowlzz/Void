...
import ac.voidac.checks.impl.badpackets.BadPacketsE;
import ac.voidac.checks.impl.badpackets.BadPacketsF;
import ac.voidac.checks.impl.badpackets.BadPacketsG;
import ac.voidac.checks.impl.badpackets.BadPacketsH;
import ac.voidac.checks.impl.elytra.ElytraC;
...
public class PacketPlayerRespawn extends PacketListenerAbstract {
        ...
        if (event.getPacketType() == PacketType.Play.Server.RESPAWN) {
            ...
            player.latencyUtils.addRealTimeTask(player.lastTransactionSent.get() + 1, () -> {
                ...
                if (player.getClientVersion().isOlderThan(ClientVersion.V_1_14)) { // 1.14+ players send a packet for this, listen for it instead
                    ...
                    // ViaVersion discards keep-all-metadata on respawn; compensate by resetting sprint state
                    player.compensatedEntities.hasSprintingAttributeEnabled = false;
                }
                player.pose = Pose.STANDING;
                player.clientVelocity = new Vector3dm();
                if (!VoidAPI.INSTANCE.getSpectateManager().isSpectating(player.uuid)) {
    ...
    private boolean isWorldChange(VoidPlayer player, WrapperPlayServerRespawn respawn) {
