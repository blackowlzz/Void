...
import ac.voidac.utils.data.TrackerData;
import ac.voidac.utils.data.packetentity.DashableEntity;
import ac.voidac.utils.data.packetentity.PacketEntity;
import ac.voidac.utils.data.packetentity.PacketEntityHook;
...
public class PacketEntityReplication extends Check implements PacketCheck {
    ...
    public PacketEntityReplication(VoidPlayer player) {
        super(player);
    }
    ...
    public void onPacketSend(PacketSendEvent event) {
        // ensure void is the one that sent the transaction
        ...
        // 1.19.3+
            ...
            // ViaVersion fakes levitation effects incorrectly, which would flag the anticheat
            ...
            if (slot.getWindowId() == 0) {
                Runnable task = () -> {
                    if (slot.getSlot() - 36 == player.packetStateData.lastSlotSelected && (
                        ...
                        if (player.isResetItemUsageOnItemUpdate() && hand == VoidAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer)) {
                            VoidAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                        }
                    }
                };
                ...
                Runnable task = () -> {
                    if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_8)) {
                        ...
                        if (items.getItems().size() > 45 && !player.inventory.getOffHand().is(items.getItems().get(45).getType())) {
                            ...
                            if (player.isResetItemUsageOnItemUpdate() && VoidAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.OFF_HAND) {
                                VoidAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                            }
                        }
                        ...
                        if (!player.inventory.getHeldItem().is(items.getItems().get(player.packetStateData.lastSlotSelected + 36).getType())) {
                            ...
                            if (player.isResetItemUsageOnItemUpdate() && VoidAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.MAIN_HAND) {
                                VoidAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                            }
                        }
                    }
        ...
        // 1.8 clients fail to send the RELEASE_USE_ITEM packet when a window is opened client sided while using an item
            ...
            if (!attach.isLeash()) {
                // Alright, let's convert this to the 1.9+ format to make it easier for void
    ...
    private void handleMoveEntity(PacketSendEvent event, int entityId, double deltaX, double deltaY, double deltaZ, Float yaw, Float pitch, boolean isRelative, boolean hasPos) {
        ...
        if (data != null) {
            ...
            if (isRelative) {
                ...
                // This causes impossible hits, so void must replace this with a teleport entity packet
    ...
    private boolean isDirectlyAffectingPlayer(VoidPlayer player, int entityID) {
        // The attributes for this entity is active, currently
