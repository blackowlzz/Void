package ac.voidac.player;

import ac.voidac.VoidAPI;
import ac.voidac.api.AbstractCheck;
import ac.voidac.api.VoidUser;
import ac.voidac.api.PacketWorld;
import ac.voidac.api.config.ConfigManager;
import ac.voidac.api.handler.ResyncHandler;
import ac.voidac.checks.Check;
import ac.voidac.checks.impl.aim.processor.AimProcessor;
import ac.voidac.checks.impl.misc.ClientBrand;
import ac.voidac.checks.impl.misc.TransactionOrder;
import ac.voidac.checks.impl.packetorder.PacketOrderProcessor;
import ac.voidac.events.packets.CheckManagerListener;
import ac.voidac.manager.*;
import ac.voidac.manager.player.features.FeatureManagerImpl;
import ac.voidac.manager.player.handlers.DefaultResyncHandler;
import ac.voidac.manager.player.handlers.NoOpResyncHandler;
import ac.voidac.platform.api.player.PlatformPlayer;
import ac.voidac.predictionengine.EntityFluidInteraction;
import ac.voidac.predictionengine.MovementCheckRunner;
import ac.voidac.predictionengine.PointThreeEstimator;
...
import ac.voidac.utils.data.*;
import ac.voidac.utils.data.packetentity.PacketEntity;
import ac.voidac.utils.data.packetentity.PacketEntityHappyGhast;
import ac.voidac.utils.data.packetentity.PacketEntityNautilus;
import ac.voidac.utils.data.packetentity.PacketEntitySelf;
import ac.voidac.utils.data.tags.SyncedTags;
import ac.voidac.utils.enums.FluidTag;
import ac.voidac.utils.enums.Pose;
import ac.voidac.utils.latency.*;
import ac.voidac.utils.math.VoidMath;
import ac.voidac.utils.math.Location;
import ac.voidac.utils.math.TrigHandler;
...
public class VoidPlayer implements VoidUser {
    ...
    public final StorageEspDecoyManager storageEspDecoyManager;
    @Getter @Setter private ResyncHandler resyncHandler = VoidAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("disable-default-resync-handler", false) ? NoOpResyncHandler.INSTANCE : new DefaultResyncHandler(this);
    ...
    public VoidPlayer(@NotNull User user) {
        ...
        storageEspDecoyManager = new StorageEspDecoyManager(this);
    ...
    public void onPacketCancel() {
        if (spamThreshold != -1 && cancelledPackets.incrementAndGet() > spamThreshold) {
            ...
            disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, VoidAPI.INSTANCE.getConfigManager().getDisconnectClosed())));
    ...
    public void sendTransaction(boolean async) {
        ...
        if (disableVoid && (System.nanoTime() - getPlayerClockAtLeast()) > 15e9) {
            return;
        }
        ...
        try {
            ...
            // Packet write may fail if the channel is closed mid-send (e.g. ProtocolLib/ViaVersion pipeline teardown)
        }
    }
    ...
    public void timedOut() {
        disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, VoidAPI.INSTANCE.getConfigManager().getDisconnectTimeout())));
    }
    ...
    public void disconnect(Component reason) {
        ...
        if (platformPlayer != null) {
            VoidAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, VoidAPI.INSTANCE.getVoidPlugin(),
    ...
    public void pollData() {
        ...
        if (!VoidAPI.INSTANCE.getPlayerDataManager().shouldCheck(user)) {
            VoidAPI.INSTANCE.getPlayerDataManager().remove(user);
        }
        ...
        if (uuid != null && this.platformPlayer == null) {
            this.platformPlayer = VoidAPI.INSTANCE.getPlatformPlayerFactory().getFromUUID(uuid);
            updatePermissions();
        }
        ...
        if (uuid != null) {
            VoidAPI.INSTANCE.getDataStoreLifecycle().sessionTracker()
    ...
    public void updatePermissions() {
        ...
        try {
            VoidAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, VoidAPI.INSTANCE.getVoidPlugin(), () -> {
                this.noModifyPacketPermission = platformPlayer.hasPermission("void.nomodifypacket");
                this.noSetbackPermission = platformPlayer.hasPermission("void.nosetback");
                storageEspDecoyManager.syncFromCurrentPermissions();
    ...
    public int getTransactionPing() {
        return VoidMath.floor(transactionPing / 1e6);
    }
    ...
    public final void reload(ConfigManager config) {
        ...
        storageEspDecoyManager.reload(config);
        ...
        maxTransactionTime = VoidMath.clamp(config.getIntElse("max-transaction-time", 60), 1, 180);
    ...
    public void resyncPositions(SimpleCollisionBox box) {
        this.resyncHandler.resync(VoidMath.floor(box.minX), VoidMath.floor(box.minY), VoidMath.floor(box.minZ),
                VoidMath.ceil(box.maxX), VoidMath.ceil(box.maxY), VoidMath.ceil(box.maxZ));
    }

    public void addMovementThisTick(VoidPlayer.Movement movement) {
        if (this.movementThisTick.size() >= 100) {
            VoidPlayer.Movement movement1 = this.movementThisTick.removeFirst();
            VoidPlayer.Movement movement2 = this.movementThisTick.removeFirst();
            VoidPlayer.Movement movement3 = new VoidPlayer.Movement(movement1.from(), movement2.to());
