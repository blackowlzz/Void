...
public final class RingRegistry {
    ...
    public <E> void register(@NotNull Category<E> cat, @NotNull Backend backend) throws BackendException {
        ...
        ThreadFactory tf = namedDaemon("void-datastore-" + backend.id() + "-" + cat.id());
    ...
    private static <E> E resetIfKnown(E event) {
        if (event instanceof ac.voidac.api.storage.event.ViolationEvent v) { v.reset(); return (E) v; }
        if (event instanceof ac.voidac.api.storage.event.SessionEvent s) { s.reset(); return (E) s; }
        if (event instanceof ac.voidac.api.storage.event.PlayerIdentityEvent p) { p.reset(); return (E) p; }
        if (event instanceof ac.voidac.api.storage.event.SettingEvent s) { s.reset(); return (E) s; }
        return event;
    }
    ...
    private long shutdownOne(Entry<?> e, long drainTimeoutMs) {
        try {
            ...
            logger.log(Level.WARNING,
                    "[void-datastore] ring " + e.category.id() + " shutdown timed out with "
