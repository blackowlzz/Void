...
import ac.voidac.command.render.HistoryComponentRenderer;
...
public class VoidHistory implements BuildableCommand {
                ...
                applyFilterFlags(commandManager,
                        commandManager.commandBuilder("void", "voidac", "void", "voidac")
                                ...
                                .flag(commandManager.flagBuilder("detailed").withAliases("d")
                                        .withDescription(Description.of("Show each violation as a separate trace line.")))
                                .flag(commandManager.flagBuilder("verbose").withAliases("v")
                                        .withDescription(Description.of("Reveal the raw verbose trace inline on each line."))))
    ...
    private static Command.Builder<Sender> applyFilterFlags(
        ...
        return b
                .flag(commandManager.flagBuilder("name")
                        ...
                        .withDescription(Description.of("Filter violations by check name regex.")))
                .flag(commandManager.flagBuilder("match")
                        .withComponent(StringParser.stringParser())
                        .withDescription(Description.of("Filter violations by verbose regex.")))
                .flag(commandManager.flagBuilder("grep")
                        .withComponent(StringParser.stringParser())
                        .withDescription(Description.of("Filter violations by check name or verbose regex.")));
    }
    ...
    private void handleDetailDefaultPage(CommandContext<Sender> context) {
        ...
        runWithPrelude(sender, target, (uuid, displayName, lifecycle, history) -> {
            ...
            if (ordinal == null) {
                sender.sendMessage(message("void-history-session-not-found",
    ...
    private static @Nullable Predicate<ViolationEntry> parseFilterFromContext(Sender sender, CommandContext<Sender> ctx) {
        ...
        try {
            ...
            sender.sendMessage(MessageUtil.miniMessage("<red>The void could not parse --" + e.flag + ": " + e.detail));
            return FILTER_ERROR;
        }
    ...
    private void runWithPrelude(Sender sender, String target, HistoryAction action) {
        ...
        if (!lifecycle.isEnabled()) {
            sender.sendMessage(message("void-history-disabled",
                    "%prefix% &cHistory subsystem is disabled!", Map.of()));
            return;
        }
        if (!lifecycle.isLoaded()) {
            sender.sendMessage(message("void-history-load-failure",
        ...
        try {
            OfflinePlatformPlayer targetPlayer =
                    VoidAPI.INSTANCE.getPlatformPlayerFactory().getOfflineFromName(target);
            ...
            sender.sendMessage(message("void-history-failed",
    ...
    private void renderDetail(Sender sender, DataStoreLifecycle lifecycle, HistoryService history,
        ...
        if (history instanceof ac.voidac.internal.storage.history.HistoryServiceImpl impl) {
     ...
     * <p>
     * Permission-gated by the surrounding {@code void.history} permission on
    ...
    private static SuggestionProvider<Sender> targetSuggestions(CloudCommandAdapter adapter) {
        ...
        return SuggestionProvider.blocking((ctx, in) -> {
            ...
            DataStoreLifecycle dsl = VoidAPI.INSTANCE.getDataStoreLifecycle();
    ...
    private static SuggestionProvider<Sender> violationPageSuggestions() {
        return SuggestionProvider.blocking((ctx, in) -> {
            ...
            try {
                ...
                if (!(dsl.historyService() instanceof ac.voidac.internal.storage.history.HistoryServiceImpl impl)) {
                    return List.of();
                }
    ...
    private static @Nullable UUID resolveTargetUuid(CommandContext<Sender> ctx) {
        ...
        OfflinePlatformPlayer p = VoidAPI.INSTANCE.getPlatformPlayerFactory().getOfflineFromName(target);
    ...
    private Component message(String key, String fallback, Map<String, String> vars) {
        String raw = VoidAPI.INSTANCE.getConfigManager().getConfig().getStringElse(key, fallback);
