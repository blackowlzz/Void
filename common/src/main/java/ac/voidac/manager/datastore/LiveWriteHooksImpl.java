...
public final class LiveWriteHooksImpl implements LiveWriteHooks {
    ...
    public void onJoinFromUserLogin(@NotNull PlatformPlayer player, @NotNull User user, long now) {
        VoidPlayer gp = VoidAPI.INSTANCE.getPlayerDataManager().getPlayer(user);
    ...
    @Override
    public void observeBrandFromCheck(@NotNull VoidPlayer voidPlayer) {
        UUID uuid = voidPlayer.user.getUUID();
        if (uuid == null) return;
        observeBrand(uuid, System.currentTimeMillis(), LiveWriteHooks.clientMetaFor(voidPlayer.user, voidPlayer));
    }
    ...
    private int resolveCheckId(@NotNull AbstractCheck check) {
        ...
        if (declaredStable != null && !declaredStable.isEmpty()) {
            ...
            if (missingStableKeyLogged.add(key)) {
                LogUtil.warn("[void-history] check " + display
    ...
    private static @Nullable String safePluginVersion() {
        try {
            var plugin = VoidAPI.INSTANCE.getVoidPlugin();
