...
public interface LiveWriteHooks {
    ...
    /** Convenience for {@code PacketPlayerJoinQuit.onUserLogin}. Builds meta from the User + VoidPlayer internally. */
    void onJoinFromUserLogin(@NotNull PlatformPlayer player, @NotNull User user, long now);

    /** Convenience for {@code PlayerDataManager.onDisconnect}. Builds meta internally; tolerates null voidPlayer (early-disconnect race). */
    void onQuitFromUserDisconnect(@NotNull User user, @Nullable VoidPlayer voidPlayer, long now);

    /** Convenience for {@code ClientBrand.onPacketReceive}. Pulls UUID + builds meta from the VoidPlayer internally. */
    void observeBrandFromCheck(@NotNull VoidPlayer voidPlayer);
     ...
     * for {@code user}. Reads from the passed-in {@link VoidPlayer} (when
    ...
    static @NotNull SessionTracker.ClientMeta clientMetaFor(
            @NotNull User user, @Nullable VoidPlayer gp) {
        String voidVersion = VoidAPI.INSTANCE.getExternalAPI().getVoidVersion();
        int clientVersionPvn = user.getClientVersion().getProtocolVersion();
        String serverVersion = VoidAPI.INSTANCE.getPlatformServer().getPlatformImplementationString();
        String brand = null;
        if (gp != null) {
            ClientBrand check = gp.checkManager.getPacketCheck(ClientBrand.class);
            if (check != null && check.isHasBrand()) brand = check.getBrand();
        }
        return new SessionTracker.ClientMeta(voidVersion, brand, clientVersionPvn, serverVersion);
    }
    ...
    LiveWriteHooks NOOP = new LiveWriteHooks() {
        ...
        @Override public void onQuitFromUserDisconnect(@NotNull User u, @Nullable VoidPlayer g, long t) {}
        @Override public void observeBrandFromCheck(@NotNull VoidPlayer g) {}
        @Override public void recordFlagFromCheck(@NotNull VoidPlayer p, @NotNull AbstractCheck c, double v, @Nullable String vb) {}
    };
}
