 ...
 * Void config file. Each yml file owns:
 *
 * <ul>
...
public final class VoidConfigSpecs {
    ...
    private VoidConfigSpecs() {}

    /**
     * Spec for the main {@code config.yml}.
     *
     * <p>v9 → v10: migrates Void 2.x's {@code history:} block out of
     ...
     *
     * <p>v10 → v11: introduces the global auto-punish block under
     * {@code punishments.auto-punish}. Old configs are upgraded by copying
     * the new bundled default in place, then auto-lifting any existing user
     * edits on top of it.
     *
     * <p>v11 → v12: shortens the public auto-punish broadcast so chat only
     * says that Void punished a player; the detailed reason remains only in
     * the punish message shown to the punished player.
      *
    * <p>v14 → v15: reserves the storage-ESP section as a disabled
    * placeholder under {@code storage-esp-decoys} while the module is off.
     ...
     * placeholder into a full configurable section. No value migration needed.
     *
     * <p>v17 → v18: adds StorageESP protection blocks to {@code AntiXray.hidden-blocks},
    ...
    public static @NotNull ConfigUpdater.Spec mainConfig() {
          return ConfigUpdater.Spec.builder("/config/", 21, ConfigUpdater.ConfigFlavor.V2)
                ...
                .migration(12, ctx -> {
                    String oldBroadcast = "&8[&5Void&8] &5The Void&7 punished a new cheater: &f%player% &8(&f%check_name%&8) &7%reason%";
                    String currentBroadcast = ctx.input().getString("punishments.auto-punish.broadcast-message");
                    if (oldBroadcast.equals(currentBroadcast)) {
                        ctx.output().put("punishments.auto-punish.broadcast-message", "&8[&5Void&8] &5The Void&7 punished &f%player%&7.");
                    }
                })
                .build();
    }
