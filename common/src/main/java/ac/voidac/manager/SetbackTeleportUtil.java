...
import ac.voidac.checks.impl.badpackets.BadPacketsN;
...
import ac.voidac.utils.data.TeleportAcceptData;
...
import ac.voidac.utils.nmsutil.ReachUtils;
...
public class SetbackTeleportUtil extends Check implements PostPredictionCheck {
    ...
    private static final VoidTeleportEvent.Channel TELEPORT_CHANNEL = VoidAPI.INSTANCE.getEventBus().get(VoidTeleportEvent.class);
    private static final VoidPlayerSetbackEvent.Channel PLAYER_SETBACK_CHANNEL = VoidAPI.INSTANCE.getEventBus().get(VoidPlayerSetbackEvent.class);
    ...
    public SetbackTeleportUtil(VoidPlayer player) {
        super(player);
    }
    ...
    public void executeForceResync() {
        if (player.gamemode == GameMode.SPECTATOR || player.disableVoid)
    ...
    private boolean isExempt() {
        ...
        if (player.disableVoid) return true;
    ...
    private void sendSetback(SetBackData data) {
        ...
        try {
                ...
                if (player.compensatedEntities.serverPlayerVehicle != null) {
                    ...
                    VoidAPI.INSTANCE.getScheduler().getEntityScheduler().execute(player.platformPlayer, VoidAPI.INSTANCE.getVoidPlugin(), () -> {
                        if (player.platformPlayer != null) {
                            VoidEntity vehicle = player.platformPlayer.getVehicle();
                            if (vehicle != null) {
                                vehicle.eject();
                            }
                        }
            ...
            if (PacketEvents.getAPI().getServerManager().getVersion().isOlderThanOrEquals(ServerVersion.V_1_7_10)) {
                y += 1.62; // 1.7 teleport offset if void ever supports 1.7 again
            }
            ...
            // Dual fire: packet-level signal for anticheat compat (VoidTeleportEvent),
            // semantic signal for admin/observability consumers (VoidPlayerSetbackEvent).
    ...
    public boolean insideUnloadedChunk() {
        Column column = player.compensatedWorld.getChunk(VoidMath.floor(player.x) >> 4, VoidMath.floor(player.z) >> 4);

        // If true, the player is in an unloaded chunk
        return !player.disableVoid && (column == null || column.transaction() >= player.lastTransactionReceived.get() ||
