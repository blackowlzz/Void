...
public final class AlertManagerImpl implements AlertManager, ConfigReloadable, StartableInitable {
    ...
    @Override
    public void start() {
        platformServer = VoidAPI.INSTANCE.getPlatformServer();
    ...
    public void reload(ConfigManager config) {
        ...
        AlertType.BRAND.disableMessage = config.getStringElse("brands-disabled", "%prefix% &7Client brands swallowed.");
    }

    /**
     * Gets the non-null PlatformPlayer from a VoidUser.
     * @throws IllegalArgumentException if the user is not a VoidPlayer.
     * @throws NullPointerException if the VoidPlayer's platformPlayer is null.
     */
    private @NotNull PlatformPlayer requirePlatformPlayerFromUser(@NotNull VoidUser user) {
        Objects.requireNonNull(user, "user cannot be null"); // Should be guaranteed by interface contract, but good practice

        if (!(user instanceof VoidPlayer voidPlayer)) {
            // Throw a specific exception if the type is wrong
            throw new IllegalArgumentException("AlertManager action called with non-VoidPlayer user: " + user.getName());
        }

        PlatformPlayer platformPlayer = voidPlayer.platformPlayer;
    ...
    @Override
    public boolean hasAlertsEnabled(@NotNull VoidUser player) {
        return hasAlertsEnabled(requirePlatformPlayerFromUser(player));
    }

    @Override
    public void setAlertsEnabled(@NotNull VoidUser player, boolean enabled, boolean silent) {
        setAlertsEnabled(requirePlatformPlayerFromUser(player), enabled, silent);
    }

    @Override
    public boolean hasVerboseEnabled(@NotNull VoidUser player) {
        return hasVerboseEnabled(requirePlatformPlayerFromUser(player));
    }

    @Override
    public void setVerboseEnabled(@NotNull VoidUser player, boolean enabled, boolean silent) {
        setVerboseEnabled(requirePlatformPlayerFromUser(player), enabled, silent);
    }

    @Override
    public boolean hasBrandsEnabled(@NotNull VoidUser player) {
        VoidPlayer voidPlayer = (VoidPlayer) player;
        ...
        if (voidPlayer.platformPlayer == null) return false;

        return hasBrandsEnabled(voidPlayer.platformPlayer);
    }

    @Override
    public void setBrandsEnabled(@NotNull VoidUser player, boolean enabled, boolean silent) {
    ...
    public boolean hasAlertsEnabled(Player player) {
        ...
        return hasAlertsEnabled(VoidAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleAlerts(Player player) {
        if (player == null) return;
        toggleAlerts(VoidAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public boolean hasVerboseEnabled(Player player) {
        if (player == null) return false;
        return hasVerboseEnabled(VoidAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleVerbose(Player player) {
        if (player == null) return;
        toggleVerbose(VoidAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }
