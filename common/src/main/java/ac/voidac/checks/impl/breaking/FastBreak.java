...
import ac.voidac.utils.nmsutil.BlockBreakSpeed;
...
@CheckData(name = "FastBreak", stableKey = "void.breaking.fast_break", description = "Breaking blocks too quickly")
public class FastBreak extends Check implements BlockBreakCheck {
    ...
    public FastBreak(VoidPlayer playerData) {
        super(playerData);
    }
    ...
    public void onBlockBreak(BlockBreak blockBreak) {
        ...
        if (blockBreak.action == DiggingAction.FINISHED_DIGGING && targetBlockPosition != null) {
            ...
            // Reset start time: 1.14.4+ has desync in block breaking netcode
    ...
    private void clampBalance() {
        ...
        blockBreakBalance = VoidMath.clamp(blockBreakBalance, -balance, balance); // Clamp not Math.max in case other logic changes
        blockDelayBalance = VoidMath.clamp(blockDelayBalance, -balance, balance);
    }
}
