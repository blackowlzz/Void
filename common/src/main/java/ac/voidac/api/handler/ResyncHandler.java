 ...
 *   <li>Targeted block resyncs after cancelled block breaks flagged by Void’s block breaking checks</li>
 *   <li>Packet-driven world updates in environments without backend world access (e.g. proxies)</li>
 * </ul>
 *
 * <p><strong>Default behavior:</strong></p>
 * Void provides an internal fallback implementation that pulls block data from the platform's backend world
 ...
 *   <li>{@link #resyncPosition} is only used internally by Void when a player fails or flags one of Void’s
...
public interface ResyncHandler {
     ...
     * <p>
     * This method is only used internally by Void when a player attempts to break a block
     * and fails a block break check (e.g., nuker, speed mine, reach). If the block break is
     * cancelled, Void will call this method to restore the correct block on the client and
