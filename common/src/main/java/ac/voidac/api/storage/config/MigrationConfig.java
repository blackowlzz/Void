 ...
 * first boot after an operator upgrades from a pre-v1 Void build.
 * <p>
 * V0 is the legacy history layout: a single {@code violations.sqlite} file
 * written by older Void versions (pre-database-redesign), populated through
 ...
 * through the {@link ac.voidac.api.storage.DataStore} event ring and read
 * by {@link ac.voidac.api.storage.history.HistoryService}.
 * <p>
 * When an operator boots Void with this feature for the first time and a V0
 * file is present in the plugin data folder, Void walks the V0 rows and
